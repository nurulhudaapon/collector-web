//! From: https://gist.github.com/karlseguin/e8e74cf5260435ec7cbf3e4c71605722
...
pub fn main() !void {
    ...
    const status = if (fail + leak == 0) Status.pass else Status.fail; // CHANGED: treat leaks as fail
    ...
    try slowest.display();
    Printer.fmt("\n", .{});
    std.posix.exit(if (fail + leak == 0) 0 else 1); // CHANGED: treat leaks as fail
}
